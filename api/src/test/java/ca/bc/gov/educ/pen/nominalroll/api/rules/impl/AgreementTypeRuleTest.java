...
public class AgreementTypeRuleTest {
  ...
  public void validate(String agreementType, final int size, String fieldError) {
    ...
    if(fieldError != null){
      assertThat(result.get(HeaderNames.LEA_PROV.getCode())).isEqualTo(String.format(fieldError, agreementType));
    }else {
      assertThat(result.get(HeaderNames.LEA_PROV.getCode())).isNull();
    }
