package ca.bc.gov.educ.pen.nominalroll.api.orchestrator.base;
...
import ca.bc.gov.educ.pen.nominalroll.api.struct.v1.NotificationEvent;
...
import static ca.bc.gov.educ.pen.nominalroll.api.constants.EventOutcome.INITIATE_SUCCESS;
import static ca.bc.gov.educ.pen.nominalroll.api.constants.EventOutcome.SAGA_COMPLETED;
...
public abstract class BaseOrchestrator<T> implements EventHandler, Orchestrator {
  ...
  protected void markSagaComplete(final Event event, final Saga saga, final T sagaData, final String payloadToSubscribers) {
    ...
    if (this.shouldSendNotificationEvent) {
      ...
      if(saga.getNominalRollStudentID() != null) {
    ...
    log.info("Saga {} is fresh", freshSaga.get());
